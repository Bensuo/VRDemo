/* stb_image_resize - v0.92 - public do
image resizing
...
// or otherwise provide the m_Index of the alpha channel. Flags value
...
//     * csFilter can be selected explicitly
//     * uint16 image type
...
{
    // The 0 m_Index of the decode buffer starts after the margin. This makes
...
static float* stbir__get_ring_buffer_entry(float* ring_buffer, int m_Index, int ring_buffer_length)
{
    return &ring_buffer[m_Index * ring_buffer_length];
}
    ...
    switch (decode)
    {
        case STBIR__DECODE(STBIR_TYPE_UINT8, STBIR_COLORSPACE_LINEAR):
            for (x=0; x < num_pixels; ++x)
            {
                ...
                for (n = 0; n < channels; n++)
                {
                    int m_Index = pixel_index + n;
                    ((unsigned char*)output_buffer)[m_Index] = STBIR__ENCODE_LINEAR8(encode_buffer[m_Index]);
                }
            }
            break;
        ...
        case STBIR__DECODE(STBIR_TYPE_UINT8, STBIR_COLORSPACE_SRGB):
            for (x=0; x < num_pixels; ++x)
            {
                ...
                {
                    int m_Index = pixel_index + nonalpha[n];
                    ((unsigned char*)output_buffer)[m_Index] = stbir__linear_to_srgb_uchar(encode_buffer[m_Index]);
                }
        ...
        case STBIR__DECODE(STBIR_TYPE_UINT16, STBIR_COLORSPACE_LINEAR):
            for (x=0; x < num_pixels; ++x)
            {
                ...
                {
                    int m_Index = pixel_index + n;
                    ((unsigned short*)output_buffer)[m_Index] = STBIR__ENCODE_LINEAR16(encode_buffer[m_Index]);
                }
            }
            break;
        ...
        case STBIR__DECODE(STBIR_TYPE_UINT16, STBIR_COLORSPACE_SRGB):
            for (x=0; x < num_pixels; ++x)
            {
                ...
                {
                    int m_Index = pixel_index + nonalpha[n];
                    ((unsigned short*)output_buffer)[m_Index] = (unsigned short)STBIR__ROUND_INT(stbir__linear_to_srgb(stbir__saturate(encode_buffer[m_Index])) * 65535);
                }
        ...
        case STBIR__DECODE(STBIR_TYPE_UINT32, STBIR_COLORSPACE_LINEAR):
            for (x=0; x < num_pixels; ++x)
            {
                ...
                {
                    int m_Index = pixel_index + n;
                    ((unsigned int*)output_buffer)[m_Index] = (unsigned int)STBIR__ROUND_UINT(((double)stbir__saturate(encode_buffer[m_Index])) * 4294967295);
                }
            }
            break;
        ...
        case STBIR__DECODE(STBIR_TYPE_UINT32, STBIR_COLORSPACE_SRGB):
            for (x=0; x < num_pixels; ++x)
            {
                ...
                {
                    int m_Index = pixel_index + nonalpha[n];
                    ((unsigned int*)output_buffer)[m_Index] = (unsigned int)STBIR__ROUND_UINT(((double)stbir__linear_to_srgb(stbir__saturate(encode_buffer[m_Index]))) * 4294967295);
                }
        ...
        case STBIR__DECODE(STBIR_TYPE_FLOAT, STBIR_COLORSPACE_LINEAR):
            for (x=0; x < num_pixels; ++x)
            {
                ...
                {
                    int m_Index = pixel_index + n;
                    ((float*)output_buffer)[m_Index] = encode_buffer[m_Index];
                }
            }
            break;
        ...
        case STBIR__DECODE(STBIR_TYPE_FLOAT, STBIR_COLORSPACE_SRGB):
            for (x=0; x < num_pixels; ++x)
            {
                ...
                {
                    int m_Index = pixel_index + nonalpha[n];
                    ((float*)output_buffer)[m_Index] = stbir__linear_to_srgb(encode_buffer[m_Index]);
                }
